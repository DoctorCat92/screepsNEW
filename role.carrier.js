...
run: function(creep) {
        ...
        if (creep.ticksToLive > 50) {
            ...
            if (Energy < EnergyCapacity) {
                ...
                if((creep.carry.energy == 0) && (total == 0)) { 
                    ...
                    var terminal = creep.room.find(FIND_STRUCTURES, {filter: { structureType: STRUCTURE_TERMINAL }});
                    ...
                    if(links.length >= 3) { 
                            ...
                            if (links[2].energy > 0) {
                                ...
                                if (storage.length > 0) {
                                    if (storage[0].store[RESOURCE_ENERGY] > 0) {
      ...
      /**  if (creep.memory.timeSay == undefined) {
            creep.memory.timeSay = 0;
        }
        if (creep.memory.timeSay == 2) {
            creep.memory.timeSay = 0;
            return creep.say('matter');
        }
        if (creep.memory.timeSay == 1) {
            creep.memory.timeSay = creep.memory.timeSay + 1 ;
            return creep.say('lives');
        }
        if (creep.memory.timeSay == 0) {
            creep.memory.timeSay = creep.memory.timeSay + 1 ;
            return creep.say('Creeps');
        } **/
}
